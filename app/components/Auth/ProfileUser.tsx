      ...
      <div className="bg-[#2A2A2A] rounded-xl w-full max-w-md border border-gray-600 overflow-hidden animate-fade-in-up">
        <div className="p-6 relative">
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <h3 className="text-xl font-bold text-white">{user?.name || 'Usuario'}</h3>
            <p className="text-gray-400">{user?.email || 'correo@correo.com'}</p>
            <div className="mt-2">
              <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${user?.rol === 'admin'
                  ? 'bg-green-100 text-green-800 border border-green-200 hover:bg-green-200'
                  : 'bg-blue-900 text-blue-100 border border-blue-700 hover:bg-blue-800'
                }`}>
                {user?.rol === 'admin' ? <MdAdminPanelSettings size={14} /> : <MdPerson size={14} />}
                {user?.rol ? user.rol.charAt(0).toUpperCase() + user.rol.slice(1) : 'User'}
              </span>
            </div>
          </div>
          ...
          {showPasswordForm ? (
            ...
            <div className="space-y-4">
              <div className="p-4 bg-[#1E1E1E] rounded-lg">
                ...
                <p className="text-white">Miembro desde: {formatDate(user?.createdAt || '')}</p>
              </div>

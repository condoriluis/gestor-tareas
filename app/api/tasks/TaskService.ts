...
export class TaskService {
    ...
    static async updateTaskStatus(id_user: number, rol_user: string, id_task: number, status_task: string, date_start_task: string, date_completed_task: string) {
        const connection = await connectDB(); 
        try {
            if (rol_user === 'admin') {
                const [rows]: any = await connection.query(
                    'UPDATE tasks SET status_task = ?, date_start_task = ?, date_completed_task = ? WHERE id_task = ?',
                    [status_task, date_start_task, date_completed_task, id_task]
                );
                return rows.affectedRows;
            } else {
                const [rows]: any = await connection.query(
                    'UPDATE tasks SET status_task = ?, date_start_task = ?, date_completed_task = ? WHERE id_task = ? AND id_user_task = ?',
                    [status_task, date_start_task, date_completed_task, id_task, id_user]
                );
                return rows.affectedRows;
            }
